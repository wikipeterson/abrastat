...
function MeanNullDistPlot({ values, diffObs, alternative, showNormalCurve = false, forceHistogram = false }: {
  ...
  }, [alternative, diffObs, showHistogram, values, values.length, xLo, xRange])
      ...
      <g transform={`translate(${MG.l},${MG.t})`}>
        ...
        {ticks.map((v, i) => (
          <g key={i} transform={`translate(${xOf(v)},${PH})`}>
            ...
            <text y={18} textAnchor="middle" fontSize={tickFontSize} fill="var(--color-muted)" fontFamily="DM Sans,sans-serif">{formatTick(v, xRange)}</text>
          </g>
        ))}
        ...
        <text x={PW/2} y={PH+30} textAnchor="middle" fontSize={axisLabelFontSize} fill="var(--color-muted)" fontFamily="DM Sans,sans-serif">Simulated x̄₁ − x̄₂</text>
      </g>
    </svg>
  )
}
...
export function TwoMeanRandomizationTest({ cardId, config, onClearZone, onAssignZone }: Props) {
  ...
  const pValueTooltip = useMemo((): string => {
    ...
    if (alternative === 'greater') return `P(x̄₁* − x̄₂* ≥ ${diffStr}) under H₀ ${pStr}`
    if (alternative === 'less') return `P(x̄₁* − x̄₂* ≤ ${diffStr}) under H₀ ${pStr}`
    return `P(|x̄₁* − x̄₂*| ≥ |${diffStr}|) under H₀ ${pStr}`
      ...
      <div className="flex-1 min-h-0 flex gap-4">
        ...
        <div className="relative flex-1 min-w-0 min-h-0">
          {simCount === 0 && !overlayVisible ? (
            ...
            <MeanNullDistPlot values={nullDist} diffObs={data?.diffObs ?? 0} alternative={alternative} showNormalCurve={showNormalCurve} forceHistogram={cardStage === 'conclude'}/>
          )}
